...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>{
    ...
    const namesOfFuncs = map((f:Sexp) => f[0], bindings);
    const parsedNameOfFuncs = mapResult(parseVarDecl,namesOfFuncs);

    const bodiesResult = mapResult(bodyMethod => parseL5CExp(second(bodyMethod)),bindings);

    const methodsResult = safe2((pNameOfFuncs: VarDecl[], bodies: CExp[]) => 
                                makeOk(zipWith(makeBinding, pNameOfFuncs, bodies)))
                                (parsedNameOfFuncs, bodiesResult);                                
    const parsedVarDecls = mapResult(parseVarDecl,varDecls);

    const typeNameStr = typeName.toString();
    return safe2((pVarDecls: VarDecl[],methods: Binding[]) => 
                    makeOk(makeClassExp(makeTVar(typeNameStr),pVarDecls, methods)))
                    (parsedVarDecls,methodsResult);
}
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => 
    isProgram(p) ? p.exps.reduce((acc:ClassExp[],exp:Exp)=>
                            acc.concat(getClassExpFromExp(exp)),[]) :
    getClassExpFromExp(p);

export const getClassExpFromExp = (exp: Exp): ClassExp[] => 
    isDefineExp(exp) ? getClassExpFromCExp(exp.val) :
    getClassExpFromCExp(exp);
export const getClassExpFromCExp = (exp: CExp): ClassExp[] => 
    isProcExp(exp) ? exp.body.reduce((acc:ClassExp[],elem:CExp)=>acc.concat(getClassExpFromCExp(elem)),[]) :
    isIfExp(exp) ?  getClassExpFromCExp(exp.test).concat(
                    getClassExpFromCExp(exp.then)).concat(
                    getClassExpFromCExp(exp.alt)) :
    isAppExp(exp) ? getClassExpFromCExp(exp.rator).concat(
                    exp.rands.reduce((acc:ClassExp[],elem:CExp)=>acc.concat(getClassExpFromCExp(elem)),[])) :
    isLetExp(exp) ? getClassExpFromBindings(exp.bindings).concat(
                    exp.body.reduce((acc:ClassExp[],elem:CExp)=>acc.concat(getClassExpFromCExp(elem)),[])) :
    isLetrecExp(exp) ?  getClassExpFromBindings(exp.bindings).concat(
                        exp.body.reduce((acc:ClassExp[],elem:CExp)=>acc.concat(getClassExpFromCExp(elem)),[])) :
    isSetExp(exp) ? getClassExpFromCExp(exp.val) :
    isClassExp(exp) ? [exp].concat(getClassExpFromBindings(exp.methods)) :
    [];

export const getClassExpFromBindings = (bindings: Binding[]): ClassExp[] =>{
    const CExps = map(b=>b.val,bindings);
    return CExps.reduce((acc:ClassExp[],elem:CExp)=>acc.concat(getClassExpFromCExp(elem)),[]);
}
//<cexp> ::= <number>                        / NumExp(val:number)
//         |  <boolean>                       / BoolExp(val:boolean)
//         |  <string>                        / StrExp(val:string)
