...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    const classArray = A.parsedToClassExps(parsed);
    const emptyTEnv = E.makeEmptyTEnv();
    return classArray.reduce(makeExtendTEnvWithClass,emptyTEnv);
}
export const makeExtendTEnvWithClass = (acc:E.TEnv,curr:A.ClassExp): E.TEnv=> 
    E.makeExtendTEnv([curr.typeName.var], [A.classExpToClassTExp(curr)],acc)
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const variable = exp.var;
    const value = exp.val;
    const varTE = variable.texp;

    const extendEnv = makeExtendTEnv([variable.var],[varTE],tenv);
    const constraints = bind(typeofExp(value, extendEnv),
                            (valueTE: TExp) => checkEqualType(varTE, valueTE, exp));                                        
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => {
    ...
    if(A.isDefineExp(exp)) { 
        return bind(typeofDefine(exp,tenv),
                    _ =>{const extendEnv = makeExtendTEnv([exp.var.var],
                                                          [exp.var.texp],
                                                           tenv);
                         return typeofProgramExps(first(exps),rest(exps),extendEnv)});
    }
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const eqTE = safe2((newTE:TExp,oldTE:TExp)=>checkEqualType(oldTE,newTE,exp))
                         (typeofExp(exp.val,tenv),applyTEnv(tenv,exp.var.var));
    return bind(eqTE, _ => makeOk(makeVoidTExp()));
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const extendTEnv = exp.fields.reduce(makeExtendTEnvWithFields,tenv);
    const methodsVarTEs = R.map((method: A.Binding) =>method.var.texp, exp.methods);
    const methodsVals = R.map((method:A.Binding)=>method.val,exp.methods);
    const methodsValTEsResult = mapResult((methodVal:A.CExp)=>typeofExp(methodVal,extendTEnv),methodsVals);
    
    const constraint = bind(methodsValTEsResult,(methodsValTEs:T.TExp[])=> checkEqualTypes(methodsValTEs, methodsVarTEs, exp));
    const fieldsTEs = R.map((field: A.VarDecl) => field.texp, exp.fields);
    return bind(constraint, _ => makeOk(T.makeProcTExp(fieldsTEs, A.classExpToClassTExp(exp))));

};

export const makeExtendTEnvWithFields = (accTEnv:E.TEnv,currField:A.VarDecl): E.TEnv=> 
    E.makeExtendTEnv([currField.var],[currField.texp],accTEnv)
